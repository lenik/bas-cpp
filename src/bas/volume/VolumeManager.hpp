...
class VolumeManager {
    ...
    using VolumeTransformer = std::function<std::shared_ptr<Volume>(std::shared_ptr<Volume>)>;
  ...
  public:
     ...
     */
    bool addVolume(std::shared_ptr<Volume> volume);
    void removeVolume(size_t index);
    void clear();

    size_t getVolumeCount() const;
    std::shared_ptr<Volume> getVolume(size_t index) const;
    std::shared_ptr<Volume> getDefaultVolume() const;

    const std::vector<std::shared_ptr<Volume>>& all() const;
    std::vector<std::shared_ptr<Volume>> type(std::string_view type) const;
     ...
     * @param excludeLoops when true (default), skip loopback devices (/dev/loop*, major 7).
     */
    void addLocalVolumes(bool includeSymbols = false, bool excludeReadOnly = false,
                         bool excludeLoops = true);
