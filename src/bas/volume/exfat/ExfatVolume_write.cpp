...
void ExfatVolume::writeDirectoryEntries(uint32_t dirCluster, const std::vector<std::vector<uint8_t>>& entries) {
    ...
    auto readClusterBlock = [&](uint32_t cluster) -> std::vector<uint8_t> {
        ...
        if (!readAt(off, buf.data(), buf.size())) {
            throw IOException("writeDirectoryEntries", m_device->name(), "Failed to read directory cluster");
        }
        return buf;
    };
            ...
            if (!foundEmpty) {
                ...
                if (firstByte == 0xE5) {
                    ...
                    if (!hasDeletedCandidate && deletedRunLen >= requiredSlots) {
                        ...
                        // We can keep scanning until empty marker to ensure candidate is truly before it,
                        // but we don't need to update it (keep earliest).
                    }
                } else {
                    deletedRunLen = 0;
                }
    ...
    if (foundEmpty) {
        ...
        for (size_t ci = emptyStartClusterIdx; ci < chain.size(); ++ci) {
            ...
            for (size_t si = startSi; si < slotsPerCluster; ++si) {
                const size_t slotOff = si * ENTRY_SLOT_SIZE;
                if (block[slotOff] != 0x00) break;
                emptyRunLen++;
                if (emptyRunLen >= requiredSlots) break;
            }
            if (emptyRunLen >= requiredSlots) break;
        }
        ...
        while (availableSlots < requiredSlots) {
            ...
            if (!writeAt(clusterToOffset(newCluster), zeroCluster.data(), zeroCluster.size())) {
                throw IOException("writeDirectoryEntries", m_device->name(), "Failed to initialize directory cluster");
            }
    ...
    for (size_t i = 0; i < neededClusters; ++i) {
        ...
        if (i == 0) firstNewCluster = newCluster;
        chain.push_back(newCluster);
        prev = newCluster;
    }
