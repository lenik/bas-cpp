...
class ExfatFileInputStream : public RandomInputStream {
public:
    ExfatFileInputStream(std::shared_ptr<BlockDevice> device, std::vector<uint32_t> clusterChain,
                         uint64_t dataOffset, uint32_t clusterSize, uint64_t fileSize);
    ~ExfatFileInputStream() override;
