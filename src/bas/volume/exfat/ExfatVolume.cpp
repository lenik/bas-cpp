...
void ExfatVolume::parseBootSector() {
    ...
    const uint64_t fatOffsetBytes = static_cast<uint64_t>(m_fatOffset) * m_bytesPerSector;
    const uint32_t fatEntries = m_clusterCount + 2;
    const uint64_t bitmapOffsetBytes = clusterToOffset(m_bitmapCluster);
    m_clusterManager = std::make_unique<ExfatClusterManager>(m_device, fatOffsetBytes, fatEntries,
                                                             bitmapOffsetBytes, m_bitmapLength);
}
...
uint32_t ExfatVolume::getFatEntry(uint32_t cluster) const {
    return m_clusterManager->getFat(cluster);
}

void ExfatVolume::setFatEntry(uint32_t cluster, uint32_t value) {
    m_clusterManager->setFat(cluster, value);
}
...
bool ExfatVolume::exists(std::string_view path) const { return ensurePathIndexed(path); }
...
bool ExfatVolume::stat(std::string_view path, DirNode* status) const {
    if (!status)
        throw std::invalid_argument("ExfatVolume::stat: status is null");
...
bool ExfatVolume::ensurePathIndexed(std::string_view path) const {
    ...
    if (normalized.empty() || normalized == "/")
        return true;
    if (m_dirents.find(normalized) != m_dirents.end())
        return true;
    ...
    while (pos < normalized.size()) {
        ...
        if (childIt == it->second.children.end())
            return false;
...
void ExfatVolume::parseDirectoryCluster(std::string_view dirPath, uint32_t firstCluster) const {
    ...
    for (uint32_t cluster : readClusterChain(firstCluster)) {
        ...
        if (!m_device->read(offset, block.data(), block.size()))
            continue;
            ...
            if (type == ENTRY_TYPE_FILE) {
                ...
                if (currentName.empty()) {
                    for (int i = 0; i < nameLength && i < 30; i++) {
                        ...
                        if (ch != 0)
                            currentName += static_cast<char>(ch);
                    }
                }
            }
        }
    ...
    if (hasCurrent) {
        std::string fullPath =
            (dirPath == "/") ? ("/" + currentName) : (std::string(dirPath) + "/" + currentName);
        m_dirents[fullPath] = currentDirent;
        parent.children[currentName] = &m_dirents[fullPath];
    }
