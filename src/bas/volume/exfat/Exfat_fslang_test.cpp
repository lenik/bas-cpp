...
#define PREFIX "Exfat_fslang_test_"
...
int main() {
    ...
    std::cout << "=== exFAT FSLang Test Suite ===\n\n";
    ...
    // Run read-only test
    {
        std::cout << "\n--- Running exfat_readonly.fsl ---\n";
        
        auto device = createMemDevice(imageBuffer.data(), imageSize);
        ExfatVolume vol(device);
        
        std::string script = readFile((testDir / "exfat_readonly.fsl").string());
        ...
        if (result.success) {
            std::cout << "✓ exfat_readonly.fsl PASSED (" << commitCount << " commits)\n";
            testsPassed++;
        } else {
            std::cerr << "✗ exfat_readonly.fsl FAILED at line " << result.failedLine << "\n";
    ...
    std::cout << "\n===========================================\n";
    std::cout << "exFAT FSLang Test Suite Complete\n";
