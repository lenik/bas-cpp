...
int main() {
    const fs::path tmpBase = fs::temp_directory_path() / ("Fat32Volume_test_" + std::to_string(::getpid()));
    ...
    assert(run_cmd("mcopy -i \"" + image.string() + "\" \"" + host.string() + "\" ::HELLO.TXT") == 0);

    Fat32Volume vol(image.string());
    assert(vol.getClass() == "fat32");

    bool foundHello = false;
    auto list = vol.readDir("/", false);
    for (const auto& e : list) {
        if (e->name == "hello.txt" || e->name == "HELLO.TXT" || e->name == "hello") {
            foundHello = true;
            break;
        }
    }
