...
#define PREFIX "Ext4_fslang_test_"
...
int main() {
    ...
    std::cout << "=== ext4 FSLang Test Suite ===\n\n";
    ...
    std::cout << "Loading ext4 image into memory...\n";
    ...
    // Run basic test
    {
        std::cout << "\n--- Running ext4_basic.fsl ---\n";
        
        auto device = createMemDevice(imageBuffer.data(), imageSize);
        Ext4Volume vol(device);
        
        std::string script = readFile((testDir / "ext4_basic.fsl").string());
        ...
        if (result.success) {
            std::cout << "✓ ext4_basic.fsl PASSED (" << commitCount << " commits)\n";
            testsPassed++;
        } else {
            std::cerr << "✗ ext4_basic.fsl FAILED at line " << result.failedLine << "\n";
    ...
    std::cout << "\n===========================================\n";
    std::cout << "ext4 FSLang Test Suite Complete\n";
