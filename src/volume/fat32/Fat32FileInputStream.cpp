...
int Fat32FileInputStream::read() {
    uint8_t b = 0;
    return (read(&b, 0, 1) == 1) ? b : -1;
}

size_t Fat32FileInputStream::read(uint8_t* buf, size_t off, size_t len) {
    if (!buf || len == 0 || m_pos >= m_fileSize) {
        return 0;
    }
    const size_t n = static_cast<size_t>(std::min<uint64_t>(len, m_fileSize - m_pos));
    if (!readAt(m_pos, buf + off, n)) {
        return 0;
    }
    m_pos += n;
    return n;
...
int64_t Fat32FileInputStream::skip(int64_t len) {
    if (len <= 0) {
        return 0;
    }
...
int64_t Fat32FileInputStream::position() const {
    return static_cast<int64_t>(m_pos);
}

bool Fat32FileInputStream::seek(int64_t offset, std::ios::seekdir dir) {
    int64_t target = 0;
    if (dir == std::ios::beg) {
        target = offset;
    } else if (dir == std::ios::cur) {
        target = static_cast<int64_t>(m_pos) + offset;
    } else if (dir == std::ios::end) {
        target = static_cast<int64_t>(m_fileSize) + offset;
    } else {
        return false;
    }
    if (target < 0) {
