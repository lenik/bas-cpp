...
public:
    ...
    size_t readFile(uint8_t* buf, size_t off, size_t len, int64_t file_offset = 0, std::ios::seekdir seek_dir = std::ios::beg) const;
    size_t writeFile(const uint8_t* buf, size_t off, size_t len, bool append = false) const;
    ...
    void writeFileLines(const std::vector<std::string>& lines, std::string_view encoding = "UTF-8") const;
    ...
    std::unique_ptr<Writer> newWriter(bool append = false, std::string_view encoding = "UTF-8") const;
