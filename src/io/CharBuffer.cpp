#include "CharBuffer.hpp"

CharBuffer CharBuffer::allocate(size_t capacity) {
    return CharBuffer(capacity);
}

CharBuffer::CharBuffer(size_t capacity)
    : m_data(capacity, 0)
    , m_position(0)
    , m_limit(capacity)
{
}

CharBuffer& CharBuffer::position(size_t p) {
    if (p > m_limit) p = m_limit;
    m_position = p;
    return *this;
}

CharBuffer& CharBuffer::limit(size_t l) {
    if (l > m_data.size()) l = m_data.size();
    m_limit = l;
    if (m_position > m_limit) m_position = m_limit;
    return *this;
}

CharBuffer& CharBuffer::clear() {
    m_position = 0;
    m_limit = m_data.size();
    return *this;
}

CharBuffer& CharBuffer::flip() {
    m_limit = m_position;
    m_position = 0;
    return *this;
}

CharBuffer& CharBuffer::rewind() {
    m_position = 0;
    return *this;
}

CharBuffer& CharBuffer::compact() {
    size_t rem = m_limit - m_position;
    if (rem > 0 && m_position > 0)
        for (size_t i = 0; i < rem; ++i)
            m_data[i] = m_data[m_position + i];
    m_position = rem;
    m_limit = m_data.size();
    return *this;
}

char16_t CharBuffer::get() {
    if (m_position >= m_limit) return 0;
    return m_data[m_position++];
}

CharBuffer& CharBuffer::get(char16_t* dst, size_t len) {
    size_t n = remaining();
    if (len > n) len = n;
    for (size_t i = 0; i < len; ++i)
        dst[i] = m_data[m_position++];
    return *this;
}

CharBuffer& CharBuffer::put(char16_t c) {
    if (m_position < m_limit)
        m_data[m_position++] = c;
    return *this;
}

CharBuffer& CharBuffer::put(const char16_t* src, size_t len) {
    size_t n = remaining();
    if (len > n) len = n;
    for (size_t i = 0; i < len; ++i)
        m_data[m_position++] = src[i];
    return *this;
}
