...
#
# Environment:
#   BAS_CPP_SRCDIR       project root (default: packaging/../..)
#   BAS_CPP_FORCE_LOCAL  skip can-local check (set on a build host by gh-makerelease)
set -euo pipefail

_LIBDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
BAS_CPP_SRCDIR=${BAS_CPP_SRCDIR:-$(cd "$_LIBDIR/../.." && pwd)}
...
find_build_host_file() {
    ...
    for k in $(_kind_fallbacks "$kind"); do
        for base in "$BAS_CPP_SRCDIR/.config/bas-cpp" "$HOME/.config/bas-cpp"; do
...
cmd_dump() {
    local file=${1:-}
    if [[ -z $file ]]; then
        file=$(find_build_host_file "${BAS_CPP_PACKAGING:-win32}") || {
    ...
    if ((BH_N > 0)); then
        ...
        echo "last_shell=$(last_shell "${BAS_CPP_PACKAGING:-}")"
...
cmd_run() {
    ...
    local k
    for k in $(_kind_fallbacks "$kind"); do
        echo "    $BAS_CPP_SRCDIR/.config/bas-cpp/$k.build-host" >&2
        echo "    $HOME/.config/bas-cpp/$k.build-host" >&2
    done
    return 2
}
