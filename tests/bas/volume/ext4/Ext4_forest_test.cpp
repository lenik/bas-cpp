...
int main() {
    ...
    assert(run_cmd("dd if=/dev/zero of=\"" + image.string() + "\" bs=1M count=32 status=none") ==
           0);
    ...
    std::cout << "\nPhase 1: Creating file tree...\n";
    {
        ...
        for (const auto& node : nodes) {
            ...
            try {
                ...
                if (created % 100 == 0) {
                    std::cout << "  Created " << created << "/" << (nodes.size() - 1)
                              << " nodes...\n";
                }
            ...
            try {
                ...
                if (node.isDirectory) {
                    ...
                    if (content != node.content) {
                        std::cerr << "  CONTENT MISMATCH: " << node.path << " (expected "
                                  << node.content.size() << " bytes, got " << content.size()
                                  << " bytes)\n";
                        errors++;
                    }
                }
                ...
                verified++;
                if (verified % 100 == 0) {
                    std::cout << "  Verified " << verified << "/" << (nodes.size() - 1)
                              << " nodes...\n";
                }
            } catch (const std::exception& e) {
